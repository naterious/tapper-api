...
export default (client) => (userInput) => {
  ...
  return Fluture.tryP(() => client.connect())
    .chain(() => {
      ...
      return Fluture.tryP(() => users.find({email: userInput.email}).toArray())
      .chain((storedUsers) => {
        return r.ifElse(
          r.isEmpty,
          ...
          () => {
            ...
            return Fluture.tryP(() => bcrypt.compare(userInput.password, user.password))
              .chain((isMatch) => {
                return r.ifElse(
                  r.equals(true),
                  () => {
